...
for image in os.scandir(IMAGE_DIR):
  # Currently displaying the crop and the mask on top of it - this could help to confirm whether the cropped image is suitable, as well as the segmentation
    ...
    #if not user_accepts_mask(base_image, base_image_mask):
        #print("user declined the full image as innacurate - there are missing objects, or the instances are masked wrong")
        #full_inaccurate = True
    #else:
        #print("User accepted the full image as accurate")
        #full_inaccurate = False
        ## usable_crops.append(base_image)
